...
//* Метод который переводит строку в нижний и верхний регистр toLowerCase() toUpperCase()
const productAmount = document.querySelectorAll('.wrapper__list-count')
...
//* Contains метод который позволяет проверить есть ли у элемента класс
...
basketOrder.addEventListener('click', () => {
    ...
    for(const key in product){
        ...
        printFooter.innerHTML = `Общая стоимость: ${totalSumProduct()} сум`
        setTimeout(() => {
